function [loss] = func_fit_lya_mpc_limit_loss_fmincon(Jz_train, z_train, f_target, H_vec, V, alphas)
%FUNC_FIT_LYA_MPC_LIMIT_LOSS_FMINCON Compute the loss for MPC-based diffeomorphic
%Lyapunov search
...
%   loss            Scalar indicating the loss
    ...
    % Calculate normalized gradients and loss
    dVdx_curr = func_grad_V_Jzz(V, Jz_train, z_train);
    loss = norm(dVdx_curr, 2)*20;
