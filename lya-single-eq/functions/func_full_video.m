function [] = func_full_video(phipath, outpath)
%FUNC_FULL_VIDEO Create a video over the individual layers of a diffeo to
%visualize how the Lyapunov function is deformed. A "full" video also plots
%the movement of the center points as opposed to "func_small_video.m".
%Note: this script also saves the first and last frames of the video as
%separate .jpg files. These are put before and after the videos in the
%final presentation.
...
%   -               (The script saves all outputs to files)
    ...
    filename_firstframe = strcat(outpath, "_firstframe.png");
    filename_lastframe = strcat(outpath, "_lastframe.png");
    ...
    % Cell array to hold the individual trajectories of the center points
    center_trajs = {};
    for i=1:n_data
        center_trajs{end+1} = x_data(:, i);
    end
    ...
    for i=1:n_layers
        ...
        % Propagate datapoints/centers through diffeo
        z_data = func_phi_forward_partial(x_data, phi, 1, i);
        % Append datapoints/centers to center trajectories
        for j=1:n_data
            center_trajs{j} = [center_trajs{j} z_data(:, j)];
        end
        ...
        plot(data_pos_full(1, :), data_pos_full(2, :), "k", "LineWidth", 2, "DisplayName", "Demonstration");
        ...
        % Save initial frame
        if i==1
            saveas(gcf, filename_firstframe);
        end
        % Save final frame
        if i==n_layers
            saveas(gcf, filename_lastframe);
        end

        % Plot center trajectories
        % Note: These are plotted after initial/last frame to avoid them
        % appearing on the last frame
        for j=1:n_data
            plot(center_trajs{j}(1, :), center_trajs{j}(2, :), "r", "LineWidth", 0.75);
        end
