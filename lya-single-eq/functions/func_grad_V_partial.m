function [dVdx] = func_grad_V_partial(phi, x, idx_start, idx_end)
...
% 
% func_grad_V_partial(phi, x, 1, 50) is equal to func_grad_V(phi, x)
%
% func_grad_V_partial(phi, x, 1, 30) calculates the gradient of V(phi(x))
% igoring the last 20 layers
%
% func_grad_V_partial(phi, x, 20, 50) calculates the gradient of V(phi(x))
